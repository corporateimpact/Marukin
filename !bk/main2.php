			...
			for($j = 1;$j<10;$j++){
				if($sample==1){
					...
					if(isset($dArray{$h . $m . "00"}[$j])){
						if($dArray{$h . $m . "00"}[$j] != "0.0"){
//							if($j == 6){
//								$data[$j] .=get_do($dArray{$h . $m . "00"}[5],$dArray{$h . $m . "00"}[6]) . ",";
//							}else{
								$data[$j] .= $dArray{$h . $m . "00"}[$j] . ",";
//							}
						}else{
...
<body>
<div style="position: fixed;background-color:#FFF;height: 320px;">
...
<div style="position:absolute; top:-1px; left:440px">	
<!--voltage_check-->
<?php
$volcheck = file_get_contents("/home/upload/infos/battery/camera.ini");
$volcheck = str_replace("\r\n", '', $volcheck);
if (strcmp($volcheck, "NG") == 0){
	echo '<img src="img/stop.gif" alt="down." title="Down." />';
}
?>
</div>
<!--voltage_check_end-->
...
<div style="padding:330px 0px 0px 0px;">
<table width="100%">
<tr>
